/**
 * Autheur: Theo Bensaci | Date: 07:54 26.11.2025 | Description: Class use to add behavior to the
 * game socket specific for the client, for example, early packet drop
 */
...
public class ServerGameSocket extends GameSocket {
  ...
  protected boolean packetPreProcess(Packet packet) {
    switch (packet.type) {
      case PacketType.command:
        ...
        // the majority of the thoses command need to be treated in the main game loop du to
        // concurency
        return true;
      ...
      case PacketType.login:
        ...
        // the player as resend a login packet with the same info and the packet comme from the
        // right client
        ...
        Entity e =
            _game.createNewPlayer(lp.username, lp.playerColor, packet.inetAddress, packet.port);
        lp.id = e.getId();
        send(lp, packet.inetAddress, packet.port);
        return false;
  ...
  public void close() {
    ...
    // send a exit packet to all client
    Set<Map.Entry<String, ClientData>> set = _game.serverPlayers.entrySet();
    for (Map.Entry<String, ClientData> entry : set) {
      send(new ExitPacket(""), entry.getValue().address, entry.getValue().port);
    }
