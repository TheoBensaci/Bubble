/** Autheur: Theo Bensaci | Date: 18:06 12.11.2025 | Description: Client network handler */
...
public class ClientNetworkHandlerSystem extends NetworkHandlerSystem {
  ...
  private int _actualGameStateId = 0;
  ...
  public void receiveUpdate(GameSocket socket) {
    ...
    for (Packet p : buffer) {
      if (p.type == PacketType.gameState) {
        ...
        boolean overflow =
            (_actualGameStateId >= 0 && gsp.stateID < 0)
                || (_actualGameStateId < 0 && gsp.stateID >= 0);

        if (gsp.stateID <= _actualGameStateId && !overflow) return;
        _actualGameStateId = gsp.stateID;
        applyGameState(gsp);
      }
    }
  }
