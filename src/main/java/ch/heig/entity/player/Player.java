/** Autheur: Theo Bensaci | Date: 18:06 12.11.2025 | Description: Player */
...
import ch.heig.entity.simpleEffect.PlayerDeathEffect;
...
public class Player extends CollisionBody implements IDrawable, IUpdatable {
  ...
  public void kill() {
    ...
    getGame().createEntity(new PlayerDeathEffect(getPosition(), playerColor));
  }
  ...
  protected void effectUpdate(float deltaTime) {
    ...
    // bubble part leave
    if (p_actualBubble == null
        && p_bufferActualBubble != null
        && p_bufferActualBubble.isActive()) { // bubble part leave
      createBubblePart(p_bufferActualBubble);
    } else if (p_actualBubble != null
        && p_bufferActualBubble == null
        && p_actualBubble.isActive()) { // bubble part enter
  ...
  private void ghostUpdate(float deltaTime) {
    ...
    // (i know it's a visual and technicly it would be logical to put it on the draw update, but
    // trust me)
    _maxGhostPosition =
        Vector2f.lerp(_maxGhostPosition, p_position, 0.03f / ((p_onDash) ? 10 : 1) * deltaTime);
  }
  ...
  protected void movementUpdate(float deltaTime) {
    ...
    if (p_inSpaceBubble) {
      ...
      if (!p_onDash && p_actualBubble != null) {
        ...
        if (diff <= 0) {
          ...
          p_position.add(
              getPosition()
                  .sub(p_actualBubble.getPosition())
                  .normilize()
                  .mult(-1 * p_actualBubble.getDecade() * deltaTime));
        }
      }
    } else {
    ...
    p_position.add(p_direction.copy().mult(velo));
  }
  ...
  public void draw(Graphics g) {
    ...
    if (getGame().debug) {
      ...
      g.fillOval(
          (int) (p_position.x - collisionRadius),
          (int) (p_position.y - collisionRadius),
      ...
      for (int i = 0; i < _NUMBER_OF_GHOST; i++) {
        ...
        ((Graphics2D) g)
            .drawImage(
                newSprite,
                (BufferedImageOp) rop,
                (int) (ghostPos.x - recenterOffset.x),
                (int) (ghostPos.y - recenterOffset.y));
      }
    }
    ...
    ((Graphics2D) g)
        .drawImage(
            SpriteRenderUtils.rotateSprite(
                ...
                p_rotation + (Math.PI / 2)),
            SpriteRenderUtils.colorToRescaleOp(outineColor),
            (int) (p_position.x - recenterOffset.x),
            (int) (p_position.y - recenterOffset.y));
    ...
    drawNumberOfDash(
        g,
        new Vector2f(p_position.x - (recenterOffset.x / 2), p_position.y + recenterOffset.y + 10));
    ...
    if (getGame().debug) {
      ...
      g.drawLine(
          (int) (p_position.x), (int) (p_position.y), (int) (aimingVec.x), (int) (aimingVec.y));
    }
  }

  protected void drawNumberOfDash(Graphics g, Vector2f initDashNumberPos) {
    g.setColor(outineColor);
    for (int i = 0; i < p_numberDash; i++) {
      g.fillOval((int) (initDashNumberPos.x - 3), (int) (initDashNumberPos.y - 3), 6, 6);
      initDashNumberPos.add(new Vector2f(10, 0));
    }
  }
  ...
  protected void createBubblePart(SpaceBubble bubble) {
    if (getGame().window == null) return;
    Vector2f diff =
        getPosition().sub(bubble.getPosition()).normilize().mult(bubble.collisionRadius);
