/** Autheur: Theo Bensaci | Date: 09:42 19.11.2025 Description: Bullet entity */
...
public class Bullet extends CollisionBody
    implements IUpdatable, IDrawable, INetworkReceiverEntity, INetworkSenderEntity {
  ...
  public Bullet setOwner(int owner_id) {
    _owner_id = owner_id;
    return this;
  }
  ...
  public void onTrigger(CollisionBody other) {
    ...
    if (other instanceof Player player) {
      if (player.getId() == _owner_id || player.dead) return;
      // kill the player >:[
      player.kill();
      getGame().destroyEntity(this);
    }
