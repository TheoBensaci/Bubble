/**
 * Autheur: Theo Bensaci | Date: 18:06 12.11.2025 | Description: Class use to manage a instance of a
 * game, it's aim to be the main loop and orginizer of the instance
 */
package ch.heig.core;

import ch.heig.core.collision.CollisionBody;
import ch.heig.core.collision.CollisionSystem;
import ch.heig.core.render.IDrawable;
import ch.heig.core.utils.Input;
import ch.heig.core.window.Window;
...
public class Game extends Thread {
  ...
  // entity gestion
  private final ArrayList<IUpdatable> _updateables =
      new ArrayList<>(); // List of entity (can really be any thing other then a Entity as long it
  // implement IUpdatable) to update every tick
  ...
  private final Semaphore _toBeCreateSemaphore =
      new Semaphore(1); // since _toBeCreate can be alter by a other thread, we need to protected
  private final Semaphore _toBeDestroySemaphore =
      new Semaphore(1); // since _toBeDestroy can be alter by a other thread, we need to protected

  private final Stack<Integer> _idPool =
      new Stack<>(); // id pool use to give a unique id to all entity needing one

  // collision
  private final CollisionSystem _collisionSystem =
      new CollisionSystem(); // Collision system use to manage collision on this instance
  ...
  /**
   * Discribe if the current game need to comport as a sever or not, check the doc for more
   * information
   *
